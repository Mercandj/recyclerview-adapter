...
open class AdapterDelegatesManager<T> {
    ...
    fun addDelegate(
        ...
        require(viewType != FALLBACK_DELEGATE_VIEW_TYPE) {
            "The view type = $FALLBACK_DELEGATE_VIEW_TYPE is reserved for fallback adapter" +
                " delegate (see setFallbackDelegate() ). Please use another view type."
        }
        require(!(!allowReplacingDelegate && delegates[viewType] != null)) {
            "An AdapterDelegate is already registered for the viewType = $viewType. " +
                "Already registered AdapterDelegate is ${delegates[viewType]}"
        }
    ...
    fun getItemViewType(items: T, position: Int): Int {
        ...
        val errorMessage = if (items is List<*>) {
    ...
    fun onCreateViewHolder(
        ...
        val delegate = getDelegateForViewType(viewType) ?: throw NullPointerException(
     ...
     * @param holder   the ViewHolder to bind viewType
     */
    @JvmOverloads
    fun onBindViewHolder(
        ...
        val delegate = getDelegateForViewType(holder.itemViewType) ?: throw NullPointerException(
            "No delegate found for item at position = $position for viewType = ${holder.itemViewType}"
        )
        delegate.onBindViewHolder(
            items = items,
            position = position,
            holder = holder,
            payloads = payloads ?: PAYLOADS_EMPTY_LIST
        )
    }
    ...
    fun onViewRecycled(holder: RecyclerView.ViewHolder) {
        val delegate = getDelegateForViewType(holder.itemViewType) ?: throw NullPointerException(
            "No delegate found for $holder for item at position = ${holder.adapterPosition} for viewType = ${holder.itemViewType}"
        )
    ...
    companion object {
        ...
        /** Used internally for [.onBindViewHolder] as empty payload parameter */
